...
class Compiler(val schema: Schema) {
    ...
    return """
      ...
      .xformat(
        ...
        'constructorParams -> schema.getFields.asScala
          .filter(_.schema.getType != Schema.Type.NULL)
          .map(compileField(_))
          .mkString(",\n")
          .indent(4),
        ...
        'hashCode -> compileRecordHashCode().indent(2))
  }
  ...
  def compileMutableRecord(): String = {
    ...
    val recordFields = schema.getFields.asScala
        .filter(_.schema.getType != Schema.Type.NULL)
  ...
  def compileRecordGet(): String = {
    def MakeFieldGetterCase(field: Schema.Field): String = {
      val converter = field.schema.getType match {
        case Schema.Type.NULL => {
          return "case %d => return null".format(field.pos)
        }
        ...
        case Schema.Type.BYTES => "// TODO(taton) Not implemented"
        case Schema.Type.FIXED => "// TODO(taton) Not implemented"
        case Schema.Type.UNION => {
          TypeMap.unionAsOption(field.schema) match {
            case None => ".getData.asInstanceOf[AnyRef]"
            case Some(_) => ".getOrElse(null).asInstanceOf[AnyRef]"
  ...
  def compileRecordHashCode(): String = {
    def makeHashCode(field: Schema.Field): String = {
      ".append(this.%(fieldName))"
          .xformat('fieldName -> field.name.toCamelCase)
    }

    val hashCodes = schema.getFields.asScala
        .filter(_.schema.getType != Schema.Type.NULL)
        .map(makeHashCode(_))
    return """
      ...
      .stripMargin
      .trim
      .xformat('hashCodes -> hashCodes.mkString("\n").indent(4))
  }
